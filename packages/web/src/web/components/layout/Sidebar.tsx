...
import {
  ...
  DollarSign, Send, AlertTriangle, ClipboardList, Upload
} from "lucide-react";
...
export function Sidebar() {
  ...
  return (
    <aside className="w-[240px] min-h-screen flex flex-col border-r border-[#1f2937] bg-[#0d1424]">
      {/* Logo */}
      ...
      <nav className="flex-1 px-3 py-4 space-y-1">
        ...
        {typeItems.map((item) => {
          const active = location + (typeof window !== "undefined" ? window.location.search : "") === item.href;
          return (
            <Link key={item.href} href={item.href}>
              <a className={cn(
                "flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all",
